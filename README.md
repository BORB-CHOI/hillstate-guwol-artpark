...
- **문의 DB 저장** — Upstash Redis(서버리스). 미설정 시 로컬 JSON 파일로 대체(개발용).
- **문의 내역 조회**
...
3. **Environment Variables**에 `.env.example` 항목을 입력합니다.
   - ⚠️ Vercel은 서버리스라 로컬 JSON 저장이 유지되지 않습니다.
     실제 운영에서는 **반드시 Upstash Redis 환경변수**를 설정하세요.
4. Deploy. 배포 후 `NEXT_PUBLIC_SITE_URL`을 실제 도메인으로 갱신하세요.

### Upstash Redis 설정 (서버리스, 콜드스타트 없음, 무료 티어)
Vercel 프로젝트 > Storage > Create Database > Upstash for Redis를 연결하면
`UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN`이 자동 주입됩니다.
(직접 만들려면 [Upstash 콘솔](https://console.upstash.com)에서 Redis 생성 후
REST URL / TOKEN을 환경변수로 등록.) 예약은 `reservations` 리스트에 JSON으로
쌓이고, `/admin`에서 최신순으로 조회합니다. 별도 스키마 설정이 필요 없습니다.
